...
- V2 `Phase 2` is implemented and locally verified on 2026-05-14, but not fully closed yet:
  ...
  - Local verification passed with `cmake --preset vs2022-debug`, `cmake --build --preset build-debug --config Debug`, and `ctest --test-dir build -C Debug --output-on-failure`.
- The remaining open item for `Phase 2` is manual standalone and VST3 basic note-playback smoke on the new allocator path. `TODO.md` is now narrowed to that check only. As of 2026-05-14, Ableton Live 12 Lite is installed locally, but REAPER was not found in the standard install locations or uninstall registry, so full two-host VST3 sign-off remains blocked until REAPER is available.
