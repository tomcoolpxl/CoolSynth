...
- V2 `Phase 2` is implemented, manually smoke-tested, and closed on 2026-05-14:
  ...
  - Local verification passed with `cmake --preset vs2022-debug`, `cmake --build --preset build-debug --config Debug`, and `ctest --test-dir build -C Debug --output-on-failure`.
- `TODO.md` now advances to V2 `Phase 3` (dual-oscillator mixer voice core). That phase now explicitly includes reducing the observed patch-dependent dense note-start click so the artifact is tracked as dry voice-core work rather than left implicit. REAPER remains part of later release-level host validation, but it is no longer a blocker for closing the allocator-only Phase 2 milestone.
